...
def parse_minimal_args(parser):
    # Dataset parameters
    parser.add_argument("--dir", default="./pretrained_embeddings/umap_embedded_datasets/USPS/", help="dataset directory")
    ...
    parser.add_argument(
        "--save_checkpoints", type=bool, default=True
    )
    parser.add_argument(
        "--exp_name", type=str, default="USPS__1"
    )
    parser.add_argument(
        "--use_labels_for_eval",
        action = "store_true",
...
def run_on_embeddings_hyperparams(parent_parser):
    ...
    parser.add_argument(
        "--init_k", default=10, type=int, help="number of initial clusters"
    )
    parser.add_argument(
        "--clusternet_hidden",
        type=int,
...
def train_cluster_net():
    ...
    train_loss=trainer.callback_metrics['cluster_net_train/train/cluster_loss']
    avg_loss=trainer.callback_metrics['cluster_net_train/train/avg_cluster_loss']
    train_loss=train_loss.numpy()
    avg_loss=avg_loss.numpy()
    plt.plot(train_loss,color='red',label='train_loss')
    plt.plot(avg_loss,color='green',label='avg_loss')
    plt.xlabel('epoch')
    plt.ylabel('loss')
